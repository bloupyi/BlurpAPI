...
public class HSQLDBDatabase implements Database {
    ...
    public HSQLDBDatabase(String filePath, String user, String pass) {
        // hsqldb.tx=mvcc pour MVCC, lock_file=false pour éviter le .lck
        this.url = "jdbc:hsqldb:file:" + filePath + ";hsqldb.tx=mvcc;hsqldb.lock_file=false";
        this.user = user;
        this.pass = pass;
    }
    ...
    public void disconnect() throws Exception {
        if (isConnected()) {
            // shutdown pour bien écrire sur disque
            try (PreparedStatement ps = connection.prepareStatement("SHUTDOWN")) {
                ps.execute();
            }
            connection.close();
        }
